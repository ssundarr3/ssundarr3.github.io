...
// game parameters
var picsPath = "assets/images/tetrispics/", emptyTile = "X", imageExtension = ".png", tetrisWrapDiv = "tetrisWrap", tetrisDiv = "tetris", tetrisTile = "tetrisTile", tetrisRow = "tetrisRow", tetrisTextDiv = "tetrisText";
...
function setButtonFunctions() {
    var rowColTileButton = document.getElementById("tetrisRowColTileForm");
    var tetrisAddNewPieceButton = document.getElementById("tetrisAddNewPieceButton");
    var tetrisGameSpeedButton = document.getElementById("tetrisGameSpeedButton");
    var autoPilotButton = document.getElementById("tetrisAutoPilot");
    var textNotification = document.getElementById("tetrisTextNotification");
    if (!rowColTileButton || !tetrisAddNewPieceButton || !tetrisGameSpeedButton || !autoPilotButton || !textNotification)
        return;
    var textNotifyP = textNotification;
    rowColTileButton.onclick = function () {
        var rowEle = document.getElementById("tetrisRowInput");
        var colEle = document.getElementById("tetrisColInput");
        var tileEle = document.getElementById("tetrisTileInput");
        if (!rowEle || !colEle || !tileEle)
            return;
        var r = +rowEle.value, c = +colEle.value, t = +tileEle.value;
    ...
    tetrisAddNewPieceButton.onclick = function () {
        var tetrisAddNewPieceText = document.getElementById("tetrisAddNewPiece");
        if (tetrisAddNewPieceText) {
            var str = tetrisAddNewPieceText.value;
            str = str.replace(/(\r\n|\n|\r)/gm, "");
            var partsOfStr = str.split(',');
            var sizeOfFirstRow = partsOfStr[0].length;
            var newPieceStr = [];
            for (var i = 0; i < partsOfStr.length; ++i) {
                newPieceStr.push([]);
                for (var j = 0; j < partsOfStr[i].length; ++j) {
            ...
            if (statsDisplayDiv) {
                ...
                for (var a = 0; a < pieces[pieces.length - 1][0].length; ++a) {
                    var rowDiv = document.createElement("div");
                    rowDiv.className = "tetrisRow";
                    for (var b = 0; b < pieces[pieces.length - 1][0][0].length; ++b) {
    ...
    tetrisGameSpeedButton.onclick = function () {
        var inputEle = document.getElementById("tetrisGameSpeed");
        if (inputEle) {
            var gs = +inputEle.value; // + to cast to number
            if (gs < 1 || gs > 60) {
...
function setAutopilot() {
    var buttonEle = (document.getElementById("tetrisAutoPilot"));
...
function drawLastPiece() {
    ...
    var textP = document.getElementById("tetrisTextNotification");
    if (textP)
        textP.innerHTML = "Block " + pieceThatCouldnt + " couldn't be placed<br>Game Over!<br>Press Space to restart";
